...
export default function SocialsHeader({ className }) {
  const parent = {
    show: {
      opacity: 1,
      transition: {
    ...
    <motion.div
      ...
      {socials.map((link) => (
        <motion.a
          ...
          key={link.id}
          href={link.link}
          className="w-7 h-7 inline-flex justify-center items-center  transition-all hover:scale-105 cursor-pointer ">
          <Image
            className="w-6 h-6"
